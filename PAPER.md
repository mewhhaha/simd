...
- Clause patterns: numeric literals, names, and `_`.

Not supported:

- `if`, `match` expressions, loops, lambdas, higher-order functions.
- ADTs, reductions/scans, shape-changing bulk operators.
- Top-level `let`.
...
- `Type = Scalar(Prim) | Bulk(Prim, Shape) | Record(...) | Fun(...)`
...
5. Lower normalized program to loop-oriented IR.
