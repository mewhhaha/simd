...
snapshot_dir="$root_dir/test/highlight_examples"
...
while IFS= read -r -d '' source_path; do
  base_name="$(basename "${source_path%.simd}")"
  snapshot_path="$snapshot_dir/$base_name.highlights.txt"
  actual="$("$tree_sitter_bin" query --captures --config-path "$config_path" "$query_path" "$source_path" | sed '1d')"
  if $update; then
  ...
  if [[ ! -f "$snapshot_path" ]]; then
    status=1
    echo "missing expected snapshot $snapshot_path" >&2
    continue
  fi
  expected="$(cat "$snapshot_path")"
  if [[ "$actual" != "$expected" ]]; then
    status=1
    echo "example highlight snapshot mismatch for ${base_name}.simd" >&2
    echo "expected: $snapshot_path" >&2
